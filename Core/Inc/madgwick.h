...
void accelQuatDeriv(float accel[4], float q[4], float out[4])
{
    float temp[4];
...
void accelQuat(float q[4], float quatDeriv[4], float samplingTime, float out[4])
{
    float temp[4];
    scalarMulQuat(quatDeriv, samplingTime, temp);
...
void computeReferenceMag2EarthFrame(float q[4], float mag[4], float quatDeriv[4], float out[4])
{
    ...
    out[0] = 0;
    out[1] = sqrtf(out[1] * out[1] + out[2] * out[2]);
    out[2] = 0;
}
...
// shi I need to make this a 4x3 mat T
void mat4x3Transpose(float q[4][3], float out[3][4])
{
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 3; j++)
...
void magAndAccelObjFunc(float gravObjFunc[3], float magObjFunc[3], float out[6])
{
    for (int i = 0; i < 3; i++)
    {
        out[i] = gravObjFunc[i];
        out[i + 3] = magObjFunc[i];
    }
}

void magAndAccelJacobT(float gravJacobian[4][3], float magJacobian[4][3], float out[4][6])
{
    // transpose the Jacobian gravity matrix
    float gravJacobT[3][4];
    mat4x3Transpose(gravJacobian, gravJacobT);

    // transpose the Jacobian magnetometer matrix
    float magJacobT[3][4];
    mat4x3Transpose(magJacobian, magJacobT);

    float temp[6][4];

    // Matrix stack to make a 6x4 matrix
    for (int i = 0; i < 3; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            temp[j][i] = gravJacobT[i][j];
            temp[j + 3][i] = gravJacobian[i][j];
        }
    }

    // Transpose the matrix to make a 4x6 matrix
    for (int i = 0; i < 6; i++)
    {
        for (int j = 0; j < 4; j++)
        {
...
void gradientDescentQuat(float q[4], float stepSize, float gradient[4], float out[4])
{
...
void getGyroError(float q[4], float direcOfErrorQuat[4], float out[4])
{
    ...
    quatProduct(temp, direcOfErrorQuat, out);
}
